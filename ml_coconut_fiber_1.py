...
#!pip install git+https://github.com/blent-ai/alepython.git #For alepython library
# ==== PDP for ALL variables on the final XGB-PSO model (exact split) ====
import os, math
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
line_style = {"color": "#8B5A2B", "linewidth": 2.5}
...
fig.suptitle("Partial Dependence (PDP) — Final XGB-PSO (trained on 70% exact split)", fontsize=13)
out_path = f"{OUT_DIR}/pdp_all_features.png"
fig.savefig(out_path, dpi=600, bbox_inches="tight")
plt.close(fig)

print("Saved PDP figure:", out_path)
...
fig.savefig(f"{OUT_DIR}/ale_top4.png", dpi=600, bbox_inches="tight")
plt.close(fig)

print("Saved:", f"{OUT_DIR}/pdp_top4.png", "and", f"{OUT_DIR}/ale_top4.png")

"""**PDP and ALE plots (Continuation)**"""
...
print(f"Locked 30% Test RMSE = {rmse:.3f} MPa")

# ---------- PDP for *all* features ----------
features = list(X_train.columns)              # <- ALL columns
n_feats  = len(features)
n_cols   = 3 if n_feats > 8 else 2            # nicer layout
n_rows   = math.ceil(n_feats / n_cols)

fig, axes = plt.subplots(n_rows, n_cols, figsize=(6*n_cols, 3.6*n_rows), constrained_layout=True)
axes = np.atleast_1d(axes).ravel()

# Let sklearn fill the entire grid in one go:
disp = PartialDependenceDisplay.from_estimator(
    model,
    X_train,
    features=features,                      # <- all features at once
    ax=axes[:n_feats],                      # use only as many axes as needed
    line_kw={"color": "#8B5A2B", "linewidth": 2.5}
)
...
# Unify y-scales across panels
ys = [ax.get_ylim() for ax in fig.get_axes()]
